...
import KFSFooterLinks from './KFSFooterLinks';
import KFSNewsletter from './KFSNewsletter';
import KFSSocialMedia from './KFSSocialMedia';
import KFSCopyright from './KFSCopyright';
import styles from './styles/KFSFooter.module.scss';
...
export default function KFSFooter({ data }: FooterProps) {
  return (
    <footer className={styles.footer}>
      <div className={styles.footerContainer}>
        {data?.links?.columns && data.links.columns.length > 0 && (
          <KFSFooterLinks columns={data.links.columns} />
        )}
