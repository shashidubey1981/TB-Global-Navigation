...
export const getHeaderEntries = async <T>(brandName: string, locale: string , referenceFieldPath: string[], query: { queryOperator?: string; filterQuery?: any }, limit:number=0) => {
    try {    
        ...
        console.log('brandName', `${CONTENT_TYPES.HEADER}_${brandName}`);
        const entryQuery = Stack.contentType(`${CONTENT_TYPES.HEADER}_${brandName}`)
            .entry()
            .locale(locale)
            .includeFallback()
        ...
        if (entryQuery) {
            ...
            if (query?.filterQuery?.length > 0 && query.queryOperator === 'or') { // filterQuery is an array of object consisting key:value pair
                ...
                const queries = query?.filterQuery?.map((q:any) => { 
                    if (typeof Object.values(q)?.[0] === 'string') {
                        ...
                        return Stack && Stack.contentType(`${CONTENT_TYPES.HEADER}_${brandName}`).entry().query().equalTo(Object.keys(q)?.[0], Object.values(q)?.[0] as string)
                    }
                    return Stack && Stack.contentType(`${CONTENT_TYPES.HEADER}_${brandName}`).entry().query().containedIn(Object.keys(q)?.[0], Object.values(q)?.[0] as any)
                })
            ...
            data.forEach((entry) => {
                isEditButtonsEnabled && addEditableTags(entry, `${CONTENT_TYPES.HEADER}_${brandName}`, true, locale)
            })
